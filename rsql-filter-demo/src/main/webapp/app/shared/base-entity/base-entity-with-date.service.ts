 ...
 */
@Injectable()
export abstract class BaseEntityWithDateService<T extends ClassWithId, U> {
  ...
  create(entity: T, entityName?: string): Observable<HttpResponse<T>> {
    ...
    return this.http
      .post<T>(this.resourceUrl, copy, { headers, observe: 'response' })
  ...
  update(entity: T, entityName?: string): Observable<HttpResponse<T>> {
    ...
    const headers = this.createHeaders(entityName);
    return this.http
      .put<T>(`${this.resourceUrl}/${entity.id}`, copy, { headers, observe: 'response' })
      .pipe(map((res: HttpResponse<T>) => this.convertDateFromServer(res)));
  }
  ...
  partialUpdate(entity: Partial<T> & ClassWithId, entityName?: string): Observable<HttpResponse<T>> {
    ...
    const headers = this.createHeaders(entityName);
    return this.http
      .patch<T>(`${this.resourceUrl}/${entity.id}`, copy, { headers, observe: 'response' })
      .pipe(map((res: HttpResponse<T>) => this.convertDateFromServer(res)));
  }
  ...
  queryAll(req?: RequestOptions): Observable<HttpResponse<T[]>> {
    const options = createRequestOption(req, true);
    return this.http
      .get<T[]>(`${this.resourceUrl}/all`, { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<T[]>) => this.convertDateArrayFromServer(res)));
  }
  ...
  exportData(req?: RequestOptions): Observable<HttpResponse<T[]>> {
    ...
    return this.http
      .get<T[]>(this.resourceExportUrl, { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<T[]>) => this.convertDateArrayFromServer(res)));
  }
  ...
  lov(req?: RequestOptions): Observable<HttpResponse<T[]>> {
    ...
    return this.http
      .get<T[]>(this.resourceLovUrl, { params: options, observe: 'response' })
      .pipe(map((res: HttpResponse<T[]>) => this.convertDateArrayFromServer(res)));
  }
